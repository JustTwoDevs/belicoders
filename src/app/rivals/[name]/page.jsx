...
async function getRival(name, router) {
  ...
  try {
    ...
    if (response.status === 404) return router.push("/404");
...
export default function Rival({ params }) {
  ...
  function handleChange() {
    async function fetchRival() {
      const foundRival = await getRival(params.name, router);
      setRival(foundRival);
    }
    fetchRival();
  }
