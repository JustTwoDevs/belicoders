...
export default function Description({ rival }) {
  const avgGrade =
    rival.grades.map((grade) => grade.value).reduce((a, b) => a + b, 0) /
    rival.grades.length;
  ...
  return (
    <section className="pt-2">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-4">
        <h1
          className={
            " text-sm font-medium text-" + rival.difficulty == 1
              ? "[#00b8a3]"
              : rival.difficulty == 2
              ? "[#fbbf24]"
              : "[#ef4444]"
        ...
        <div className="flex items-center gap-1 p-1 rounded hover:bg-primary-200 cursor-pointer">
          ...
          <h1 className="text-sm">
            {avgGrade.toFixed(2)} ({rival.grades.length} votes)
          </h1>
        </div>
        ...
        <span className="text-sm font-medium rounded p-1 hover:bg-primary-200 ">
          {new Date(rival.createdAt.$date).toLocaleDateString()}
        </span>
        <span className="text-sm font-medium rounded p-1 hover:bg-primary-200 ">
          By {rival.createdBy}
        </span>
